    ...
    for current_doc in tqdm(dataset.value):
        ...
        for sent in current_doc.value:
            for word in sent.value:
                t_list = list(itertools.chain.from_iterable(
                    tag_positions[word.attr['offset']:word.attr['offset'] + len(word.value)]))
...
def construct_umls_rnn_features(dataset, umls_type_vocab=None):
    # For now this modifier should always be called after
    # add_surface_feature_list()
