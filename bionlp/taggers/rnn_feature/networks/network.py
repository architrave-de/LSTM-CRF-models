...
from .crf_lstm_layer import CRFLayer, get_crf_training_loss
import lasagne
import time
import theano
import logging
...
def setup_NN(worker, x_in, u_in, mask_in, y_in, params, numTags, emb_w):
    ...
    regularization_losses = params['l2crf'] * T.sum((crf_params)**2)
    ...
    if params['momentum'] == 0:
        logger.info('Not using any momentum')
    ...
    # draw_net.draw_to_file(sum_layer, 'network.pdf', output_shape=False, verbose=True)
