...
def constructApproximations(unary, pairwise, t_out, numTags, params, x_in, u_in, y_in, mask_in, l_in, l_mask, l_u_in, normalization=True):
    logger.info('Initializing Approximation layer with normalization as {0}'.format(
        normalization))
    ...
    forward_unary = lasagne.layers.SliceLayer(
        unary, indices=slice(0, params['maxlen'] - 1), axis=1)
    backward_unary = lasagne.layers.SliceLayer(
        unary, indices=slice(1, params['maxlen']), axis=1)
    forward_inputs = lasagne.layers.ConcatLayer(
        [forward_unary, pairwise], axis=2)
    backward_inputs = lasagne.layers.ConcatLayer(
        [backward_unary, pairwise], axis=2)
    ...
    forward1 = lasagne.layers.RecurrentLayer(forward_inputs, numTags, W_in_to_hid=lasagne.init.GlorotUniform(), W_hid_to_hid=lasagne.init.GlorotUniform(
    ), b=lasagne.init.Constant(0.), nonlinearity=lasagne.nonlinearities.rectify, hid_init=lasagne.init.Constant(0.))
    backward1 = lasagne.layers.RecurrentLayer(backward_inputs, numTags, W_in_to_hid=lasagne.init.GlorotUniform(), W_hid_to_hid=lasagne.init.GlorotUniform(
    ), b=lasagne.init.Constant(0.), nonlinearity=lasagne.nonlinearities.rectify, hid_init=lasagne.init.Constant(0.), backwards=True)
    ...
    forward_l = lasagne.regularization.regularize_layer_params(
        forward1, lasagne.regularization.l1)
    backward_l = lasagne.regularization.regularize_layer_params(
        backward1, lasagne.regularization.l1)
    ...
    def get_internal_results(deterministic):
        ...
        unary_potential = lasagne.layers.get_output(
            unary, deterministic=deterministic)
        forward_results = lasagne.layers.get_output(
            forward1, deterministic=deterministic)
        backward_results = lasagne.layers.get_output(
            backward1, deterministic=deterministic)
        backward_results = T.concatenate(
            [backward_results, T.zeros_like(backward_results[:, :1, :])], axis=1)
        forward_results = T.concatenate(
            [T.zeros_like(forward_results[:, :1, :]), forward_results], axis=1)
    ...
    m_out = theano.function(
        [l_in.input_var, l_u_in.input_var, l_mask.input_var], training_p)
    logger.debug('Log Prob size is {0}'.format(m_out(x_in.astype(
        'int32'), u_in.astype('float32'), mask_in.astype('float32')).shape))
    approx_params = lasagne.layers.get_all_params(
...
def get_crf_training_loss(Inference_Layer, pairwise, t_out, numTags, params, x_in, y_in, mask_in, l_in, l_mask):
    ...
    pairwise_sequence = lasagne.layers.get_output(
        pairwise, deterministic=False)
    pairwise_sequence = pairwise_sequence.dimshuffle(1, 0, 2)
    pair_shape = pairwise_sequence.shape
    pairwise_potential = T.reshape(
        pairwise_sequence, (pair_shape[0], pair_shape[1], numTags, numTags))
    unary_potential = lasagne.layers.get_output(
    ...
    unary_score = T.sum(unary_potential * t_out *
                        mask_out.dimshuffle(0, 1, 'x'), axis=[1, 2])
    ...
    pairwise_score_result = theano.function(
        [t_out, l_in.input_var, l_mask.input_var], pairwise_score)
    ...
    total_score = theano.function(
        [t_out, l_in.input_var, l_mask.input_var], pairwise_score + unary_score)
    zee_ = theano.function([l_in.input_var, l_mask.input_var], theano_logsumexp(
        unnormalized_log, axis=2)[:, 0])
    cost_ = theano.function([t_out, l_in.input_var, l_mask.input_var], theano_logsumexp(
