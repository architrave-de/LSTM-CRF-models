...
def trainer(config_params):
    ...
    if not config_params['noeval']:
        logger.info(
            'Evaluation on the input dataset is on ( -noeval 0). I will search for gold standard annotation in .json files ')
        raw_text, documents = annotated_file_extractor(
            deploy_params['input'], config_params['umls'])
    else:
        raw_text, documents = file_extractor(
            deploy_params['input'], config_params['umls'])
    ...
    if config_params['umls'] != 0 and 'UMLS_TYPE' not in encoded_documents.active:
        logger.info(
            'UMLS is on, but the dataset does not have umls tags. Adding and caching ..')
        encoded_documents = add_umls_type(encoded_documents)
    del raw_text
    del documents
    logger.info('Gathering stored vocabularies')
    model_file = pickle.load(
        open(config_params['model'], 'rb'), encoding='latin1')
    w2i = model_file['w2i']
    ...
    if config_params['umls'] != 0:
        encoded_documents, umls_vocab_dict = construct_umls_rnn_features(
            encoded_documents, umls_vocab_dict)
    ...
    if config_params['output-dir'] is not 'None':
        prepare_document_report(
            texts, label, pred, encoded_documents, config_params['output-dir'])
    if not config_params['noeval']:
